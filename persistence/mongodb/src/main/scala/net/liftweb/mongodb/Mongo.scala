...
case class MongoHost(server: ServerAddress = new ServerAddress, options: MongoClientOptions = MongoClientOptions.builder.build) extends MongoHostBase {
 ...
 */
case class MongoSet(dbs: List[ServerAddress], options: MongoClientOptions = MongoClientOptions.builder.build) extends MongoHostBase {
